  ...
  <div class="mod-zone" style="height: 100%;">
   ...
   <template slot="paneR">
    ...
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="getZoneList()">
      <el-form-item>
        <el-input v-model="dataForm.panZoneName" size="mini" placeholder="巡区名称" clearable></el-input>
      </el-form-item>
      <el-form-item>
        <el-button @click="search()" size="mini">查询</el-button>
        <el-button v-if="isAuth('inspection:panzone:save')" size="mini" @click="zoneAddOrUpdateHandle()">新增</el-button>
      ...
      <el-form-item>
          ...
          class="device-import"
          accept=".xlsx,.xls"
          :action="this.$http.adornUrl(`/inspection/panzone/upload?token=${this.$cookie.get('token')}`)"
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <el-button v-if="isAuth('inspection:panzone:save')" type="text" size="mini" @click="zoneAddOrUpdateHandle(scope.row.panZoneId)">修改</el-button>
          <el-button v-if="isAuth('inspection:panzone:delete')" type="text" size="mini" @click="zoneDeleteHandle(scope.row.panZoneId)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
    ...
    <!-- 弹窗, 新增 / 修改 -->
    <PanZoneAddOrUpdate v-if="panZoneAddOrUpdateVisible" ref="panZoneAddOrUpdate" @refreshDataList="getZoneList"></PanZoneAddOrUpdate>
  </div>
  <div class="show-data-down" style="flex: 1;display: flex;flex-direction: column;">
    <el-form :inline="true" :model="dataForm">
      <el-form-item>
        <el-button v-if="isAuth('inspection:panzonedevice:save')" @click="deviceAddOrUpdateHandle()" :disabled="dataForm.PanZoneId <= 0" size="mini">绑定设备</el-button>
        <el-button v-if="isAuth('inspection:panzonedevice:delete')" type="warning" @click="deviceDeleteHandle()" :disabled="deviceListSelections.length <= 0" size="mini">批量解绑</el-button>
      </el-form-item>
    </el-form>
    <el-table
      border
    ...
    <!-- 弹窗, 新增 / 修改 -->
    <PanDeviceAddOrUpdate v-if="deviceAddOrUpdateVisible" ref="deviceAddOrUpdate" @DeviceBindEvent="deviceBindEvent"></PanDeviceAddOrUpdate>
...
<script>
  ...
  import PanZoneAddOrUpdate from './panzone-add-or-update '
  import PanDeviceAddOrUpdate from './panzonedevice-add-or-update '
  ...
  export default {
    data () {
      return {
        ...
        zoneListSelections: [],
        panZoneAddOrUpdateVisible: false,
        deviceList: [],
        devicePageIndex: 1,
        devicePageSize: 5,
        deviceTotalPage: 0,
    ...
    components: {
      depttree,
      PanZoneAddOrUpdate,
      PanDeviceAddOrUpdate,
      splitPane,
      importResult
    },
    activated () {
      // 接收巡线传递过来的数据
      this.dataForm.PanZoneName = this.$route.params.PanZoneName
      this.search()
    },
    methods: {
      rowSelect(e){
        this.dataForm.PanZoneId = e.panZoneId
        console.log(e)
      },
      onDrawBack () {
      ...
      getDeviceList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/inspection/panzonedevice/list'),
          method: 'get',
          params: this.$http.adornParams({
            'page': this.devicePageIndex,
            'limit': this.devicePageSize,
            'panZoneId': this.dataForm.panZoneId
          })
        }).then(({data}) => {
          if (data && data.code === 0) {
      ...
      exportToExcel (list) {
        ...
        require.ensure([], () => {
          ...
          const tHeader = ['巡区名称<pan_zone_name>', '编码<pan_zone_code>', '所属机构<dept_name>']
          const filterVal = ['panZoneName', 'panZoneCode', 'deptName']
      ...
      exportExcelHandle () {
        this.$http({
          url: this.$http.adornUrl('/inspection/panzone/export'),
          method: 'get',
          params: this.$http.adornParams({
            'panZoneId': this.dataForm.panZoneId
          })
      ...
      importExcelHandle (list) {
        this.$http({
          url: this.$http.adornUrl('/inspection/panzone/import'),
          method: 'post',
      ...
      getZoneList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/inspection/panzone/list'),
          method: 'get',
          params: this.$http.adornParams({
            'page': this.zonePageIndex,
            'limit': this.zonePageSize,
            'panZoneName': this.dataForm.panZoneName,
      ...
      zoneSelectionChangeHandle (val) {
        if (val != null) {
          this.dataForm.panZoneId = val.panZoneId
          this.devicePageIndex = 1
          this.devicePageSize = 5
          this.deviceTotalPage = 0
          this.getDeviceList()
        } else {
          this.dataForm.panZoneId = 0
        }
      },
       // 多选
      ...
      zoneAddOrUpdateHandle (id) {
        this.panZoneAddOrUpdateVisible = true
        this.$nextTick(() => {
          this.$refs.panZoneAddOrUpdate.getDeptList()
          this.$refs.panZoneAddOrUpdate.init(id)
        })
      },
      // 绑定设备
      deviceAddOrUpdateHandle (id) {
        ...
        this.$nextTick(() => {
          this.$refs.deviceAddOrUpdate.init(this.dataForm.deptId, this.dataForm.panZoneId)
        })
      },
      ...
      deviceBindEvent (val) {
        ...
        this.$http({
          url: this.$http.adornUrl('/inspection/panzonedevice/bind'),
          method: 'post',
          params: this.$http.adornParams({
            'panZoneId': this.dataForm.panZoneId
          }),
      ...
      zoneDeleteHandle (id) {
        ...
        this.$confirm(`确定删除?`, '提示', {
          ...
          this.$http({
            url: this.$http.adornUrl('/inspection/panzone/delete'),
            method: 'post',
      ...
      deviceDeleteHandle (id) {
        ...
        this.$confirm(`确定解绑?`, '提示', {
          ...
          this.$http({
            url: this.$http.adornUrl('/inspection/panzonedevice/delete'),
            method: 'post',
            params: this.$http.adornParams({
              'panZoneId': this.dataForm.panZoneId
            }),
