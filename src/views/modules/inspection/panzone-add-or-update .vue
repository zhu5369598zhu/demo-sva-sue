<template>
  <el-dialog
    v-dialog-drag
    :title="!dataForm.panZoneId ? '新增' : '修改'"
    ...
    <el-row :gutter="5">
      <el-col :span="24">
        <el-form-item label="盘区名称" prop="panZoneName">
          <el-input v-model="dataForm.panZoneName" placeholder="盘区名称" size="mini"></el-input>
        </el-form-item>
      </el-col>
    </el-row>
    <el-row :gutter="5">
      <el-col :span="21">
        <el-form-item label="编码" prop="panZoneCode">
          <el-input v-model="dataForm.panZoneCode" placeholder="盘区编码" size="mini"></el-input>
        </el-form-item>
      </el-col>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      init (id) {
        this.dataForm.panZoneId = id || 0
        this.visible = true
        this.isHttp = false
        this.$nextTick(() => {
          this.$refs['dataForm'].resetFields()
          if (this.dataForm.panZoneId) {
            this.$http({
              url: this.$http.adornUrl(`/inspection/panzone/info/${this.dataForm.panZoneId}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.deptId = data.panZone.deptId
                this.dataForm.deptName = data.panZone.deptName
                this.dataForm.panZoneName = data.panZone.panZoneName
                this.dataForm.panZoneCode = data.panZone.panZoneCode
                this.dataForm.createTime = data.panZone.createTime
      ...
      getRfid () {
        this.$http({
          ...
          if (data && data.code === 0 && data.bind !== null) {
            this.dataForm.panZoneCode = data.bind.content
          } else {
            this.setTimer()
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.isHttp = true
            this.$http({
              url: this.$http.adornUrl(`/inspection/panzone/${!this.dataForm.panZoneId ? 'save' : 'update'}`),
              method: 'post',
              data: this.$http.adornData({
                'deptId': this.dataForm.deptId,
                'panZoneId': this.dataForm.panZoneId || undefined,
                'panZoneName': this.dataForm.panZoneName,
                'panZoneCode': this.dataForm.panZoneCode,
