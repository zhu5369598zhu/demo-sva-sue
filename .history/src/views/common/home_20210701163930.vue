    ...
    <div class="nav">
      <div class="change-type">
        <div class="type-left" :class=" +''+menuC?'showListType':' '">
          <ul>
            <li @click="gotoRouteHandle(item.list[0])" v-for="(item, index) in menuList" :key="index">
              <a>{{ item.name }}</a
              ><span></span>
            </li>
          </ul>
      ...
      <div class="user_box">
        <el-dropdown :show-timeout="0" placement="bottom">
          ...
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item @click.native="" style="color: #333"
  ...
  methods: {
    ...
    gotoRouteHandle(menu) {
      ...
      var route = this.dynamicMenuRoutes.filter(
        (item) => item.meta.menuId ===  menu.list?menu.list[0].menuId:menu.menuId
      );
      if (route.length >= 1) {
